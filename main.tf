...
locals {
  ...
  template_variables = {
    ...
    affected_sequences = local.sequences
  ...
  # Set all sequences as default value in case of an empty list is provided
  sequences_to_string = length(var.sequences) == 0 ? "ALL" : join(",", var.sequences)

  # If all schemas or all sequences are specified grant privileges for all sequences
  # otherwise concate all sequences names into single string with schema as prefix
  # jsonencode is used to quote schema and each sequence name additionally to prevent issue with hyphen usage:
  #   ['"schema_name"."sequence_name"', ...]
  sequences = (
    local.schema == "ALL" || local.sequences_to_string == "ALL" ? "ALL" :
    join(",", formatlist(
      "${jsonencode(local.schema)}.%s", [
        for sequence in var.sequences : jsonencode(sequence)
      ]
    ))
  )
