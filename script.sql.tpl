...
BEGIN
  ...
  IF affected_schema = 'ALL' THEN
    ...
    LOOP
      EXECUTE FORMAT('GRANT %s ON ALL SEQUENCES IN SCHEMA "%s" TO "${group_role}"', granted_privileges, each_schema);
  ...
  ELSEIF affected_sequences = 'ALL' THEN
    EXECUTE FORMAT('GRANT %s ON ALL SEQUENCES IN SCHEMA "%s" TO "${group_role}"', granted_privileges, affected_schema);
    RAISE NOTICE '% privileges were granted on all sequences in %.', granted_privileges, affected_schema;
  ELSE
    EXECUTE FORMAT('GRANT %s ON %s TO "${group_role}"', granted_privileges, affected_sequences);
