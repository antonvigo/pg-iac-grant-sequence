...
variable "schema" {
  description = "New privileges will be granted on sequences from this database schema, default value is 'public'. In case of 'ALL' value specified privileges will be granted for all sequences in all schemas no matter what consists in var.sequences"
  type        = string
  default     = "public"
}

variable "sequences" {
  description = "List of sequences within current schema for which specified privileges will be granted, default value is 'ALL'. There is no need to set schema as prefix"
  type        = list(string)
  default     = ["ALL"]
}
